...
interface OutcomeModalFreshIVFProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

type Step = "retrieval" | "fertilization" | "testing" | "transfer"
...
export function OutcomeModalFreshIVF({ open, onOpenChange }: OutcomeModalFreshIVFProps) {
  ...
  const getActiveSteps = (): Step[] => {
    ...
    if (formData.embryosCreated === "no") return ["retrieval", "fertilization"]
    return ["retrieval", "fertilization", "testing", "transfer"]
  }

  // Which steps are crossed out (skipped due to negative answer)
  const getSkippedSteps = (): Step[] => {
    if (formData.retrievalHappened === "no") return ["fertilization", "testing", "transfer"]
    if (formData.embryosCreated === "no") return ["testing", "transfer"]
    return []
  }
  ...
  const isReadyToSubmit = (): boolean => {
    ...
    switch (currentStep) {
      ...
      case "transfer":
        // If transfer happened, numTransferred is required
        if (formData.transferHappened === "yes") {
          return formData.numTransferred !== "" && !errors.numTransferred
        }
        return formData.transferHappened === "no"
      case "testing":
        // testing is never terminal in Fresh IVF - always proceed to transfer
        return false
      default:
        return false
    }
  ...
  const setField = (field: keyof FormData, value: string) => {
    const max = field === "numTransferred" ? OUTCOME_COPY.variants.freshIvf.transferred.max : 100
  ...
  const goNext = (updatedData?: FormData) => {
    ...
    if (currentStep === "retrieval") {
      if (data.retrievalHappened !== "no") setCurrentStep("fertilization")
    } else if (currentStep === "fertilization") {
      if (data.embryosCreated !== "no") setCurrentStep("testing")
  ...
  const goBack = () => {
    if (currentStep === "fertilization") setCurrentStep("retrieval")
    else if (currentStep === "testing") setCurrentStep("fertilization")
    else if (currentStep === "transfer") setCurrentStep("testing")
  }
  ...
  const handleFertilizationChange = (value: "yes" | "no") => {
    const updated: FormData = {
      ...formData,
  ...
  // Transfer step: auto-advance is not needed since there are optional fields
  const handleTransferChange = (value: "yes" | "no") => {
    setFormData({
      ...formData,
      transferHappened: value,
      numTransferred: "",
      numDiscarded: "",
    })
  }

  // For the testing step, submit is not terminal — the Next button advances to Transfer
  const testingCanProceed = (): boolean => {
  ...
  const canProceedFromCurrentStep = (): boolean => {
    switch (currentStep) {
      ...
      case "testing":
        return testingCanProceed()
      case "transfer":
        return false // transfer is always the last step when reached
      default:
        return false
    }
  }
        ...
        <div className="min-h-[200px] py-2">
          ...
          {currentStep === "retrieval" && (
            <div className="space-y-3">
              ...
              >
                ...
                >
                  <RadioGroupItem value="yes" id="fresh-retrieval-yes" />
                ...
                <label
                  htmlFor="fresh-retrieval-no"
                  className={cn(
                    "flex cursor-pointer items-center gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50",
                    formData.retrievalHappened === "no" && "border-primary bg-primary/5"
                  )}
                >
                  <RadioGroupItem value="no" id="fresh-retrieval-no" />
          ...
          {currentStep === "fertilization" && (
            <div className="space-y-3">
              ...
              <RadioGroup
                ...
                onValueChange={(v) => handleFertilizationChange(v as "yes" | "no")}
                className="gap-3"
              >
                <label
                  htmlFor="fresh-embryos-yes"
                  className={cn(
                    "flex cursor-pointer items-center gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50",
                    formData.embryosCreated === "yes" && "border-primary bg-primary/5"
                  )}
                >
                  <RadioGroupItem value="yes" id="fresh-embryos-yes" />
                  <span className="flex-1 text-sm">{OUTCOME_COPY.embryos.yes}</span>
                </label>
                <label
                  htmlFor="fresh-embryos-no"
                  className={cn(
                    "flex cursor-pointer items-center gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50",
                    formData.embryosCreated === "no" && "border-primary bg-primary/5"
                  )}
                >
                  <RadioGroupItem value="no" id="fresh-embryos-no" />
            ...
            <div className="space-y-3">
              ...
              {formData.geneticTesting === "report_received" && (
                <div className="grid grid-cols-2 gap-4 rounded-lg border bg-muted/30 p-4">
                  <div className="flex flex-col">
                    <div className="mb-2 min-h-[24px]">
                      <div className="flex items-center gap-1.5">
                        <Label htmlFor="fresh-num-biopsied" className="text-sm">
                  ...
                  <div className="flex flex-col">
                    <div className="mb-2 min-h-[24px]">
                      <div className="flex items-center gap-1.5">
                        <Label htmlFor="fresh-num-euploid" className="text-sm">
              ...
              {formData.geneticTesting === "no_report_yet" && (
                <div className="rounded-lg border bg-muted/30 p-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-1.5">
                      <Label htmlFor="fresh-num-biopsied-pending" className="text-sm">
                        {OUTCOME_COPY.fields.biopsied.label} <span className="text-destructive">*</span>
                      </Label>
                      <Tooltip>
                        <TooltipTrigger asChild>
              ...
              {formData.geneticTesting === "no_testing" && (
                <div className="rounded-lg border bg-muted/30 p-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-1.5">
                      <Label htmlFor="fresh-num-stored" className="text-sm">
                        {OUTCOME_COPY.variants.freshIvf.stored.label} <span className="text-destructive">*</span>
                      </Label>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <InfoIcon className="size-3.5 cursor-help text-muted-foreground" />
                        ...
                        <TooltipContent className="max-w-xs">
                          {OUTCOME_COPY.variants.freshIvf.stored.tooltip}
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <Input
          ...
          {currentStep === "transfer" && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <Label className="text-base font-medium">
                  {OUTCOME_COPY.variants.freshIvf.transfer.question} <span className="text-destructive">*</span>
                </Label>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <InfoIcon className="size-4 cursor-help text-muted-foreground" />
                  ...
                  <TooltipContent className="max-w-xs">
                    {OUTCOME_COPY.variants.freshIvf.transfer.tooltip}
                  </TooltipContent>
                </Tooltip>
              </div>
              <RadioGroup
                value={formData.transferHappened || ""}
                onValueChange={(v) => handleTransferChange(v as "yes" | "no")}
                className="gap-3"
              >
                <label
                  htmlFor="fresh-transfer-yes"
                  className={cn(
                    "flex cursor-pointer items-center gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50",
                    formData.transferHappened === "yes" && "border-primary bg-primary/5"
                  )}
                >
                  <RadioGroupItem value="yes" id="fresh-transfer-yes" />
                  <span className="flex-1 text-sm">{OUTCOME_COPY.variants.freshIvf.transfer.yes}</span>
                </label>
                <label
                  htmlFor="fresh-transfer-no"
                  className={cn(
                    "flex cursor-pointer items-center gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50",
                    formData.transferHappened === "no" && "border-primary bg-primary/5"
                  )}
                >
                  <RadioGroupItem value="no" id="fresh-transfer-no" />
                  <span className="flex-1 text-sm">{OUTCOME_COPY.variants.freshIvf.transfer.no}</span>
                </label>
              </RadioGroup>

              {formData.transferHappened === "yes" && (
                <div className="rounded-lg border bg-muted/30 p-4">
                  <div className="space-y-2">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <Label htmlFor="fresh-num-transferred" className="text-sm">
                          {OUTCOME_COPY.variants.freshIvf.transferred.label} <span className="text-destructive">*</span>
                        </Label>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <InfoIcon className="size-3.5 cursor-help text-muted-foreground" />
                          ...
                          <TooltipContent className="max-w-xs">
                            {OUTCOME_COPY.variants.freshIvf.transferred.tooltip}
                          </TooltipContent>
                        </Tooltip>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {OUTCOME_COPY.variants.freshIvf.transferred.subtext}
                      </p>
                    </div>
                    <Input
                      id="fresh-num-transferred"
                      type="number"
                      min="0"
                      max={OUTCOME_COPY.variants.freshIvf.transferred.max}
                      placeholder="0"
                      value={formData.numTransferred}
                      onChange={(e) => setField("numTransferred", e.target.value)}
                      className={cn("max-w-[160px]", errors.numTransferred && "border-destructive focus-visible:ring-destructive")}
                    />
                    {errors.numTransferred && <p className="text-xs text-destructive">{errors.numTransferred}</p>}
                  </div>
                </div>
              )}
            </div>
