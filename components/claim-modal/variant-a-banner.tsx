...
import { XIcon, SparklesIcon, LockIcon } from "lucide-react"
import { RATES, SERVICES, type OutcomeScenario } from "@/lib/claim-prefill-data"
import { cn } from "@/lib/utils"

/**
 * Variant A — Pre-fill banner + inline muted states.
 *
 * Visual idea:
 *  - Subtle sage banner at the top of the modal summarises the pre-fill.
 *  - Treatment dropdown opens pre-selected; disabled rates are shown
 *    in the dropdown list as muted with a small reason subtitle.
 *  - Billable Services lists every service; disabled ones are locked
 *    with an inline reason, pre-checked ones show a tiny "auto" tag.
 */
export function VariantABanner({ scenario }: { scenario: OutcomeScenario }) {
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-[13px] leading-relaxed text-stone-600">
              ...
              <span className="text-stone-500">
                {disabledRateCount > 0 && `${disabledRateCount} rate${disabledRateCount > 1 ? "s" : ""}`}
                {disabledRateCount > 0 && disabledServiceCount > 0 && " and "}
                {disabledServiceCount > 0 && `${disabledServiceCount} service${disabledServiceCount > 1 ? "s" : ""}`}
                {(disabledRateCount > 0 || disabledServiceCount > 0) && " unavailable for this cycle."}
              </span>
            </p>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex w-full items-center justify-between gap-2">
                        <span className={cn("text-[13px]", disabled ? "text-stone-400" : "text-stone-900")}>
                          {rate.label}
                        </span>
                        {disabled && (
                          <LockIcon className="size-3 shrink-0 text-stone-400" strokeWidth={2} />
                        )}
                      </div>
                      {disabled && reason && (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className={cn("text-sm", disabled ? "text-stone-500 line-through" : "text-stone-900")}>
                          {service.label}
                        </span>
                        {autoFilled && !disabled && (
